...
const sampleTrades = [
  ...
  { id: crypto.randomUUID(), import_batch: 'hdfc-b1', imported: true, date: '2026-01-05', time: '10:02', symbol: 'AU SMALL FINANCE BANK LIMITED', isin: '', exchange: 'NSE', product: 'MTF', delivery_type: 'D', side: 'LONG', qty: 600, entry_price: 963.87, exit_price: 958.2, stop_loss: 954, take_profit: 990, gross_value: 578322, commission: 590, total_charges: 880, stt: 188, stamp_duty: 42, net_amount: -1140, pnl: -1140, pnl_pct: -0.58, r_multiple: -0.6, status: 'LOSS', broker: 'HDFC', order_no: '1000000003623639', fills_count: 4, strategy: 'Pullback', setup_type: 'Pullback', timeframe: '5m', emotion: 'FOMO', confidence: 5, grade: 'C', tags: ['#FOMO', '#rule-break'], notes: 'Chased entry. Stopped out at LOD.', lessons: 'Wait for pullback confirmation.', rule_violations: ['Chased entry'] },
  { id: crypto.randomUUID(), import_batch: 'k-b1', imported: true, date: '2026-01-07', time: '11:10', symbol: 'Canara Bank', isin: 'INE476A01022', exchange: 'NSE', product: 'MIS', delivery_type: '', side: 'SHORT', qty: 1500, entry_price: 471.2, exit_price: 465.8, stop_loss: 474, take_profit: 464, gross_value: 706800, commission: 300, total_charges: 470, stt: 0, stamp_duty: 0, net_amount: 3200, pnl: 3200, pnl_pct: 1.15, r_multiple: 1.9, status: 'WIN', broker: 'KOTAK', order_no: '', fills_count: 2, strategy: 'Reversal', setup_type: 'Reversal', timeframe: '15m', emotion: 'FOCUSED', confidence: 7, grade: 'B+', tags: ['#banking'], notes: 'Weak open and failed retest.', lessons: 'Shorts work well on weak breadth.', rule_violations: [] },
  { id: crypto.randomUUID(), import_batch: '', imported: false, date: '2026-01-10', time: '13:30', symbol: 'Reliance', isin: '', exchange: 'NSE', product: 'CNC', delivery_type: 'D', side: 'LONG', qty: 100, entry_price: 2812, exit_price: 2798, stop_loss: 2795, take_profit: 2860, gross_value: 281200, commission: 140, total_charges: 220, stt: 80, stamp_duty: 18, net_amount: -1660, pnl: -1660, pnl_pct: -0.5, r_multiple: -0.8, status: 'LOSS', broker: 'MANUAL', order_no: '', fills_count: 1, strategy: 'News', setup_type: 'News', timeframe: '5m', emotion: 'NERVOUS', confidence: 4, grade: 'C-', tags: ['#news'], notes: 'Entered before confirmation candle.', lessons: 'Avoid anticipatory entries.', rule_violations: ['Ignored plan'] },
  { id: crypto.randomUUID(), import_batch: '', imported: false, date: '2026-01-12', time: '09:55', symbol: 'Nippon India Silver ETF', isin: '', exchange: 'NSE', product: 'CNC', delivery_type: 'D', side: 'LONG', qty: 1000, entry_price: 232.3, exit_price: 236.9, stop_loss: 229, take_profit: 238, gross_value: 232300, commission: 116, total_charges: 180.8, stt: 0, stamp_duty: 0, net_amount: 4260, pnl: 4260, pnl_pct: 1.98, r_multiple: 1.4, status: 'WIN', broker: 'KOTAK', order_no: '', fills_count: 1, strategy: 'Trend', setup_type: 'Trend', timeframe: '1h', emotion: 'CALM', confidence: 8, grade: 'A-', tags: ['#etf'], notes: 'Swing continuation.', lessons: 'Good risk definition.', rule_violations: [] },
  ...[
...
function App() {
  ...
  const saveTrade = () => {
    ...
    const rec = { ...newTrade, id: crypto.randomUUID(), import_batch: '', imported: false, date: newTrade.date.slice(0, 10), time: newTrade.date.slice(11, 16), isin: '', delivery_type: '', gross_value: newTrade.entry_price * newTrade.qty, commission: 0, total_charges: 0, stt: 0, stamp_duty: 0, net_amount: pnl, pnl, pnl_pct: (pnl / (newTrade.entry_price * newTrade.qty)) * 100, r_multiple: r, status: pnl > 0 ? 'WIN' : pnl < 0 ? 'LOSS' : 'BREAKEVEN', broker: 'MANUAL', order_no: '', fills_count: 1, grade: r > 2 ? 'A' : r > 1 ? 'B+' : 'C' };
    setTrades((p) => [rec, ...p]);
  ...
  const onImportFile = async (file) => {
    ...
    const normalized = parsed.map((p) => ({ ...newTrade, ...p, id: crypto.randomUUID(), import_batch: batch, imported: true, exit_price: p.entry_price, stop_loss: p.entry_price * 0.99, take_profit: p.entry_price * 1.02, pnl: p.net_amount || 0, pnl_pct: 0, r_multiple: 0, status: (p.net_amount || 0) > 0 ? 'WIN' : (p.net_amount || 0) < 0 ? 'LOSS' : 'BREAKEVEN', strategy: 'Imported', setup_type: 'Imported', timeframe: 'D', emotion: 'CALM', confidence: 5, grade: 'B', tags: ['#imported'], notes: 'Imported fill', lessons: '', rule_violations: [] }));
