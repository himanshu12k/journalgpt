...
function App() {
  ...
  const [importState, setImportState] = useState({ broker: 'KOTAK', preview: [], history: [], lastImport: null });
  ...
  const onImportFile = async (file) => {
    ...
    setImportState((s) => ({ ...s, broker, preview: normalized.slice(0, 10), history: [{ batch, count: normalized.length, broker }, ...s.history], lastImport: { fileName: file.name, broker, batch, count: normalized.length, importedAt: new Date().toISOString() } }));
...
`}</style>
      <div className="md:flex">
        ...
        <main className="md:ml-[220px] p-6 md:p-10 w-full overflow-auto">
          ...
          {tab === 'Import' && <div><h2 className="uppercase tracking-[3px] mb-4">⟩ BROKER IMPORT</h2><div className="grid md:grid-cols-4 gap-2">{['IBKR','TDA / TOS','SCHWAB','ROBINHOOD','WEBULL','ETRADE','TRADEZERO','GENERIC CSV','KOTAK ₹','HDFC ₹'].map((b)=><button key={b} onClick={()=>setImportState((s)=>({...s,broker:b.includes('KOTAK')?'KOTAK':b.includes('HDFC')?'HDFC':b}))} className={`border border-[var(--border)] p-2 text-left ${importState.broker===b||importState.broker===b.split(' ')[0]?'border-l-2 border-l-[var(--accent)] bg-[var(--s2)]':''}`}>{b}</button>)}</div><label className="block mt-4 border border-[var(--border)] p-6 text-center hover:border-[var(--accent)]">┌─ DROP FILE ───────────────────────────────────────────────┐<br/>drag & drop CSV / XLS / XLSX here<br/>└───────────────────────────────────────────────────────────┘<input type="file" className="hidden" accept=".csv,.xls,.xlsx,.txt" onChange={(e)=>e.target.files?.[0] && onImportFile(e.target.files[0])} /></label>{importState.lastImport && <div className="mt-4 border border-[var(--border)] p-3 bg-[var(--surface)]"><div>┌─ UPLOAD TRADE CONFIRMATION ───────────────────────────────┐</div><div className="text-[var(--green)]">› Import complete.</div><div>File: {importState.lastImport.fileName}</div><div>Broker: {importState.lastImport.broker}  Batch: {importState.lastImport.batch.slice(0, 12)}</div><div>Trades Imported: {importState.lastImport.count}</div><div>Imported At: {new Date(importState.lastImport.importedAt).toLocaleString()}</div><div>└────────────────────────────────────────────────────────────┘</div></div>}{importState.preview.length>0&&<div className="mt-4"><div>┌─ PREVIEW — {importState.preview.length} TRADES DETECTED ────────────────────────────┐</div><table className="w-full text-sm"><tbody>{importState.preview.map((p)=><tr key={p.id} className="border-b border-[var(--border)]"><td>{p.date}</td><td>{p.symbol}</td><td>{p.side}</td><td>{p.qty}</td><td>{p.entry_price.toFixed(2)}</td><td>{p.exchange}</td></tr>)}</tbody></table><div>└────────────────────────────────────────────────────────────┘</div></div>}{importState.history.length>0 && <div className="mt-4"><div className="uppercase tracking-[2px] text-[var(--muted)] text-xs mb-1">IMPORT HISTORY</div><table className="w-full text-xs"><thead><tr className="uppercase tracking-[2px] text-[var(--muted)]"><th className="text-left">Batch</th><th className="text-left">Broker</th><th className="text-left">Count</th></tr></thead><tbody>{importState.history.slice(0,5).map((h)=><tr key={h.batch} className="border-b border-[var(--border)]"><td>{h.batch.slice(0,12)}</td><td>{h.broker}</td><td>{h.count}</td></tr>)}</tbody></table></div>}</div>}
